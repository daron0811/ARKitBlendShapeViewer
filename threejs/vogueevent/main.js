...
var scenemodelFile = 'ARScene_RayFix_V11.json';
var nowStatus = 0;
...
function RotateItem(event)
{
	...
	{
		card.rotation.y += (rotateValueX / 1)*deltaTime*swipeSpeed;
	}
	oldvalueX = pointX;
	oldvalueY = pointY;
}
...
function takePhoto() {
  ...
  html2canvas(document.querySelector("#camera-view"), {
    ...
    imgCamera.onload = () => {
      ...
      $.ajax({
        type: "POST",
        url: "https://funk-ar.com/api/v1/event/vogue",
        data: {
          card_id: "VIP_Card",
...
function onClickSelectPhoto() {
  ...
  FB.ui({
    ...
    method: 'share',
    href: shareUrl,
    hashtag: "#臺北時裝週xVOGUEFNO", 

  }, function(resp) {
    gtag("event", 'VogueVIP_share_end_photo', {
      'event_label': "VogueVIP",
      'event_category': "vogue_share_end_photo",
    });  
  });
}
